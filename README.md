...
```json
{
    ...
    "is_filter": "らなの発言にフィルタリングをかけるかです。(単純なブラックリスト形式のものです)[真偽値]"
}
```
### core.js - 言語野
らなの言語野を担当するプログラムです。  
...
消すと大変なことになります。
### filter_words.json - フィルターさん
らなが暴走するのを防ぐためのフィルターです。  
フィルターに登録されているワードを発言しようとすると発言中に自主規制音が流れます。  
...
#### Q. OpenAI_API_Gokan_APIって何？
A. OpenAI API互換APIとしてらなに話しかけられるツールです。  
会話はトークンごとに分けられます。
